...
const InventoryPage: React.FC<PageProps> = ({ appData }) => {
    ...
    return (
        <div>
            ...
            <div ref={printableRef} className="bg-white dark:bg-gray-800 rounded-lg overflow-x-auto shadow-xl">
                <div className="printable-area inventory-table-container">
                        ...
                        <tbody>
                            {filteredProducts.map(product => (
                                <tr key={product.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                    ...
                                    <td className="p-4 font-semibold text-gray-800 dark:text-white">{product.name}</td>
                                    <td className={`p-4 ${product.quantity <= product.minStock ? 'text-red-500 font-bold' : 'text-gray-600 dark:text-gray-300'}`}>{product.quantity}</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{product.price.toFixed(2)} ج.م</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{product.cost.toFixed(2)} ج.م</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{product.minStock}</td>
                        ...
                        {filteredCustomers.map(customer => {
                            ...
                            return (
                                <tr key={customer.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                    <td className="p-4 font-semibold text-gray-800 dark:text-white">{customer.name}</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{customer.phone}</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{customer.address}</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{stats.operationsCount}</td>
            ...
            <div className="space-y-4">
                ...
                {stats.totalDebt > 0 && (
                     <form onSubmit={handleAddPayment} className="p-4 bg-gray-100 dark:bg-gray-700/50 rounded-lg space-y-2">
                        <h4 className="font-bold text-gray-800 dark:text-white">{t('addPayment')}</h4>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-200 dark:border-gray-600">
                    <div>
                        <h4 className="font-bold mb-2 text-gray-800 dark:text-white">{t('allCustomerRecords')}</h4>
                     ...
                     <div>
                        <h4 className="font-bold mb-2 text-gray-800 dark:text-white">{t('customerPayments')}</h4>
                        <div className="max-h-60 overflow-y-auto pr-2">
                            ...
                            ) : <p className="text-gray-600 dark:text-gray-300">{t('noPayments')}</p>}
                        </div>
                    </div>
                </div>
            </div>
...
const SalesPage: React.FC<PageProps> = ({ appData }) => {
    const { t } = useTranslation();
    const { products, customers, addSale, addInvoice, addFawrySale, addCustomer, companyInfo, printSettings } = appData;
    const { addNotification } = useNotification();
        ...
        <div>
            <div className="printable-area text-black">
                {companyInfo.logo && (
                    <div className="text-center mb-4">
                        <img 
                            ...
                            style={{ opacity: companyInfo.logoOpacity ?? 1.0 }}
                        />
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2 text-sm my-4">
                    <p><strong>{t('date')}:</strong> {formatDate(new Date().toISOString())}</p>
                    <p><strong>{t('customer')}:</strong> {selectedCustomer?.name || customerName}</p>
                    <p><strong>{t('phone')}:</strong> {selectedCustomer?.phone || 'N/A'}</p>
                    <p className="col-span-2"><strong>{t('address')}:</strong> {selectedCustomer?.address || 'N/A'}</p>
                </div>
                ...
                <div className="mt-4 pt-2 border-t text-sm space-y-1">
                     <p className="flex justify-between"><strong>{t('subtotal')}:</strong> {subtotal.toFixed(2)}</p>
                     <p className="flex justify-between"><strong>{t('discount')}:</strong> {discount.toFixed(2)}</p>
                     <p className="flex justify-between text-lg font-bold"><strong>{t('finalTotal')}:</strong> {finalTotal.toFixed(2)}</p>
                     <p className="flex justify-between"><strong>{t('amountPaid')}:</strong> {paymentStatus === PaymentStatus.Paid ? finalTotal.toFixed(2) : amountPaid.toFixed(2)}</p>
                     <p className="flex justify-between" style={{color: 'red'}}><strong>{t('amountRemaining')}:</strong> {amountRemaining.toFixed(2)}</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg space-y-6">
                    ...
                    {selectedCustomer && (
                         <div className="p-2 bg-gray-100 dark:bg-gray-700/50 rounded-md text-sm text-gray-700 dark:text-gray-300">
                            <h4 className="font-bold text-gray-800 dark:text-gray-200">{t('customerInfo')}</h4>
                    ...
                    <div className="mt-6">
                        <h3 className="font-bold text-lg mb-2 text-gray-800 dark:text-white">{t('cart')}</h3>
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col">
                    <h3 className="text-2xl font-bold mb-4 border-b border-gray-200 dark:border-gray-700 pb-2 text-gray-800 dark:text-white">{t('total')}</h3>
                    <div className="space-y-4 flex-grow text-gray-700 dark:text-gray-300">
                        ...
                        <div className="flex justify-between text-2xl font-bold text-green-500 pt-2 border-t border-gray-200 dark:border-gray-700"><span>{t('finalTotal')}:</span><span>{finalTotal.toFixed(2)} ج.م</span></div>
                        ...
                        <div className="flex justify-between text-lg text-red-500">
                            <span>{t('amountRemaining')}:</span><span>{amountRemaining.toFixed(2)} ج.م</span>
                        </div>
                    ...
                    <div className="mt-6 space-y-2">
                        <Button onClick={handlePrint} className="w-full bg-gray-600 hover:bg-gray-700"><PrintIcon/> {t('print')}</Button>
            ...
            <div className="space-y-4 text-gray-800 dark:text-gray-200">
                 <div className="printable-area text-black">
                    ...
                    <div className="mt-4 pt-2 border-t text-sm space-y-1">
                         ...
                         <p className="flex justify-between" style={{color: 'red'}}><strong>{t('amountRemaining')}:</strong> {record.amountRemaining.toFixed(2)}</p>
                    </div>
                     {printSettings.footerText && <p className="text-center text-xs mt-6">{printSettings.footerText}</p>}
                </div>
...
const ReportsPage: React.FC<PageProps> = ({ appData }) => {
    ...
    const filteredData = useMemo(() => {
        ...
        const all = Object.entries(allRecords)
            .flatMap(([type, records]) => records.map(rec => ({ ...rec, type: t(type as any) })))
            .filter(rec => {
                const recDate = new Date(rec.date);
                return recDate >= start && recDate <= end;
            }).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        
        // FIX: Renamed shadowed variable `t` to `typeKey` to avoid conflict with the translation function `t`.
        const sales = all.filter(rec => ['sales', 'invoices', 'fawrySales'].some(typeKey => rec.type.includes(typeKey) || t(typeKey as any) === rec.type));
        // FIX: Renamed shadowed variable `t` to `typeKey` to avoid conflict with the translation function `t`.
        const returns = all.filter(rec => ['salesReturns', 'invoiceReturns'].some(typeKey => rec.type.includes(typeKey) || t(typeKey as any) === rec.type));

        return { all, sales, returns };

    }, [allRecords, startDate, endDate, t]);
    
    const handleExportPDF = () => {
        const doc = new jsPDF();
        const tableColumn = [t('recordNumber'), t('type'), t('customer'), t('date'), t('total'), t('profit')];
        const tableRows: any[] = [];

        filteredData.all.forEach(record => {
            const recordData = [
                record.recordNumber,
                record.type,
                record.customerName,
    ...
    const handleExportExcel = () => {
        const worksheetData = filteredData.all.map(record => ({
            [t('recordNumber')]: record.recordNumber,
            [t('type')]: record.type,
            [t('customer')]: record.customerName,
            [t('date')]: formatDate(record.date),
            [t('total')]: record.finalTotal,
            [t('profit')]: record.profit,
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg flex items-end gap-4 no-print">
                ...
                <Button onClick={() => window.print()} className="bg-gray-600 hover:bg-gray-700">
                    <PrintIcon />
                    <span>{t('print')}</span>
                </Button>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                 <div className="flex border-b dark:border-gray-700 overflow-x-auto no-print">
                     {['overview', 'sales', 'products', 'customers', 'allTransactions'].map(tab => (
                ...
                <div className="p-6">
                    {activeTab === 'overview' && (
                     ...
                     {activeTab === 'sales' && (
                         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                             <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                                <h3 className="font-bold mb-4 text-gray-800 dark:text-white">{t('salesAndProfitOverTime')}</h3>
                                <ResponsiveContainer width="100%" height={300}>
                                    <LineChart data={salesOverTime}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#555' : '#ccc'} /><XAxis dataKey="date" tick={{ fill: theme === 'dark' ? '#fff' : '#333' }} /><YAxis tick={{ fill: theme === 'dark' ? '#fff' : '#333' }} /><Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#333' : '#fff' }} /><Legend wrapperStyle={{ color: theme === 'dark' ? '#fff' : '#333' }}/><Line type="monotone" dataKey="sales" name={t('sales')} stroke="#8884d8" /><Line type="monotone" dataKey="profit" name={t('profit')} stroke="#82ca9d" /></LineChart>
                                </ResponsiveContainer>
                            </div>
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                                <h3 className="font-bold mb-4 text-gray-800 dark:text-white">{t('paymentStatusDistribution')}</h3>
                                <ResponsiveContainer width="100%" height={300}>
                                    <PieChart><Pie data={paymentStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} fill="#8884d8" label={{ fill: theme === 'dark' ? '#fff' : '#333' }}>{paymentStatusData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#333' : '#fff' }}/><Legend wrapperStyle={{ color: theme === 'dark' ? '#fff' : '#333' }}/></PieChart>
                                </ResponsiveContainer>
                            </div>
                         </div>
                     )}
                     {activeTab === 'products' && (
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                            <h3 className="font-bold mb-4 text-gray-800 dark:text-white">{t('topSellingProducts')}</h3>
                            <ResponsiveContainer width="100%" height={400}>
                                <BarChart data={topProductsData} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#555' : '#ccc'} /><XAxis type="number" tick={{ fill: theme === 'dark' ? '#fff' : '#333' }}/><YAxis type="category" dataKey="name" width={150} tick={{fontSize: 12, fill: theme === 'dark' ? '#fff' : '#333' }}/><Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#333' : '#fff' }} /><Bar dataKey="quantity" name={t('quantity')} fill="#82ca9d" /></BarChart>
                            </ResponsiveContainer>
                        </div>
                     )}
                     {activeTab === 'customers' && (
                         <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                            <h3 className="font-bold mb-4 text-gray-800 dark:text-white">{t('topCustomers')}</h3>
                            <ResponsiveContainer width="100%" height={400}>
                                <BarChart data={topCustomersData} ><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#555' : '#ccc'} /><XAxis dataKey="name" tick={{fontSize: 10, fill: theme === 'dark' ? '#fff' : '#333' }}/><YAxis tick={{ fill: theme === 'dark' ? '#fff' : '#333' }}/><Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#333' : '#fff' }}/><Bar dataKey="total" name={t('totalPurchases')} fill="#8884d8" /></BarChart>
                            </ResponsiveContainer>
                        </div>
                     )}
                     {activeTab === 'allTransactions' && (
                         <div>
                             <div className="flex justify-end gap-2 mb-4 no-print">
                                 <Button onClick={handleExportPDF} className="bg-red-600 hover:bg-red-700"><PdfIcon /> {t('exportPDF')}</Button>
                                 <Button onClick={handleExportExcel} className="bg-green-600 hover:bg-green-700"><ExcelIcon /> {t('exportExcel')}</Button>
                             </div>
                             <div className="overflow-x-auto">
                                 <table className="w-full min-w-max text-right">
                                     <thead className="bg-gray-100 dark:bg-gray-700/50">
                                     ...
                                     <tbody className="text-gray-700 dark:text-gray-300">
                                         {filteredData.all.map(rec => (
                                             <tr key={rec.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                                 ...
                                                 <td className="p-3 text-sm">{formatDate(rec.date)}</td>
                                                 <td className="p-3">{rec.finalTotal.toFixed(2)}</td>
                                                 <td className="p-3">{rec.profit.toFixed(2)}</td>
                                                 <td className="p-3">{rec.paymentStatus}</td>
                                             </tr>
                                         ))}
                                     </tbody>
...
const UserGuideModal: React.FC<{isOpen: boolean; onClose: () => void;}> = ({isOpen, onClose}) => {
    const { t } = useTranslation();
    const GuideSection: React.FC<{title: string; content: string}> = ({title, content}) => (
        <div className="mb-4">
            <h4 className="text-xl font-bold text-blue-500 dark:text-blue-400 mb-1">{title}</h4>
            <p className="text-gray-600 dark:text-gray-300">{content}</p>
        </div>
    );
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={t('userGuideTitle')}>
            <div className="max-h-[70vh] overflow-y-auto pr-4 rtl:pl-4 rtl:pr-0 text-right">
                <GuideSection title={t('guideIntroTitle')} content={t('guideIntroContent')} />
                <GuideSection title={t('guideDashboardTitle')} content={t('guideDashboardContent')} />
                <GuideSection title={t('guideInventoryTitle')} content={t('guideInventoryContent')} />
                <GuideSection title={t('guideCustomersTitle')} content={t('guideCustomersContent')} />
                <GuideSection title={t('guideSalesTitle')} content={t('guideSalesContent')} />
                <GuideSection title={t('guideReturnsTitle')} content={t('guideReturnsContent')} />
                <GuideSection title={t('guideRecordsTitle')} content={t('guideRecordsContent')} />
                <GuideSection title={t('guideReportsTitle')} content={t('guideReportsContent')} />
                <GuideSection title={t('guideSettingsTitle')} content={t('guideSettingsContent')} />
                <GuideSection title={t('guideSupportTitle')} content={t('guideSupportContent')} />
            </div>
        </Modal>
    );
};
...
const SettingsPage: React.FC<{
    ...
    const handlePrintSettingsChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value, type } = e.target;
        const target = e.target as HTMLInputElement;

        if (type === 'checkbox') {
             setPrintForm(prev => ({ ...prev, [name]: target.checked }));
        } else if (type === 'number') {
             setPrintForm(prev => ({ ...prev, [name]: parseInt(value, 10) || 0 }));
        } else {
    ...
    const handleCompanyInfoSave = (e: React.FormEvent) => {
        ...
        updateCompanyInfo({
            ...companyForm,
            // FIX: Replaced potentially problematic '??' operator with a null check and explicit Number conversion to ensure logoOpacity is a valid number. This resolves TypeScript errors about arithmetic operations.
            logoOpacity: companyForm.logoOpacity != null ? Number(companyForm.logoOpacity) : 1.0
        });
        addNotification(t('operationSuccess'), 'success');
    }
    ...
    const handlePrintSettingsSave = (e: React.FormEvent) => {
        ...
        updatePrintSettings({
            ...printForm,
            // FIX: Replaced a complex and potentially buggy check with a robust validation to ensure fontSize is a valid finite number. This resolves TypeScript errors and correctly handles numeric strings.
            fontSize: printForm.fontSize != null && Number.isFinite(Number(printForm.fontSize)) ? Number(printForm.fontSize) : 10
        });
        addNotification(t('operationSuccess'), 'success');
    }
    ...
    return (
        <div>
            ...
            <div className="space-y-8 max-w-4xl mx-auto">
                {/* Company Info */}
                 <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                    <form onSubmit={handleCompanyInfoSave} className="space-y-4">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">{t('companyInfo')}</h3>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                {companyForm.logo && <Button type="button" onClick={() => setCompanyForm(prev => ({...prev, logo: ''}))} className="!bg-red-600 hover:!bg-red-700 !p-2 text-xs">{t('removeLogo')}</Button>}
                            </div>
                        </div>
                         <div>
                            <label htmlFor="logoOpacity" className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{t('logoOpacity')}</label>
                            {/* FIX: Provide a default value for logoOpacity to avoid controlled/uncontrolled input warnings and errors. */}
                            <input type="range" id="logoOpacity" name="logoOpacity" min="0" max="1" step="0.1" value={companyForm.logoOpacity ?? 1.0} onChange={e => setCompanyForm(prev => ({...prev, logoOpacity: parseFloat(e.target.value)}))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"/>
                 ...
                 <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                    <form onSubmit={handlePrintSettingsSave} className="space-y-4">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">{t('printSettings')}</h3>
                        <Input label={t('printFooterText')} name="footerText" value={printForm.footerText} onChange={handlePrintSettingsChange} />
                        <Input label={t('printFontSize')} name="fontSize" type="number" value={printForm.fontSize} onChange={handlePrintSettingsChange} />
                        <div className="flex items-center">
                           <input type="checkbox" id="showCostOnPrint" name="showCostOnPrint" checked={printForm.showCostOnPrint} onChange={handlePrintSettingsChange} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"/>
                           <label htmlFor="showCostOnPrint" className="ml-2 rtl:mr-2 block text-sm text-gray-900 dark:text-gray-300">{t('showCostOnPrint')}</label>
                        </div>
                ...
                {/* Appearance Settings */}
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">{t('theme')}</h3>
                ...
                {permissions.canManageUsers && (
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">{t('userManagement')}</h3>
                 ...
                 {permissions.canViewLogs && (
                     <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                         <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">{t('system')}</h3>
                        <Button onClick={() => setIsLogModalOpen(true)} className="w-full">
                            <LogIcon/>
                            <span>{t('viewActivityLog')}</span>
                        </Button>
                        <Button onClick={() => setIsGuideModalOpen(true)} className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700">
...
const AiAssistant: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && !chatRef.current) {
            try {
                ...
                setChatHistory([{role: 'model', text: "AI Assistant could not be initialized."}]);
            }
        }
    }, [isOpen]);
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await chatRef.current.sendMessage({ message: currentPrompt });
    ...
    return (
        <>
            <button
                ...
                className="fixed bottom-6 right-6 rtl:left-6 rtl:right-auto bg-purple-600 text-white rounded-full p-4 shadow-lg hover:bg-purple-700 transition-transform hover:scale-110 z-50 no-print"
                title={t('aiAssistant')}
            >
...
const App = () => {
    ...
    }, [appData.currentUser, currentPage, permissions]);
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'settings': return <SettingsPage appData={appData} theme={theme as string} setTheme={setTheme as (theme: 'light' | 'dark') => void} />;
